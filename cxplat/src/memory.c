...
cxplat_duplicate_string(_In_z_ const char* source)
{
    ...
    char* destination = (char*)cxplat_allocate(CXPLAT_POOL_FLAG_NON_PAGED, size, CXPLAT_TAG_STRING);
    if (destination) {
...
{
    cxplat_free((void*)source, CXPLAT_POOL_FLAG_NON_PAGED, CXPLAT_TAG_STRING);
}
...
cxplat_duplicate_utf8_string(_Out_ cxplat_utf8_string_t* destination, _In_ const cxplat_utf8_string_t* source)
{
    if (!source->value || !source->length) {
        ...
        destination->value =
            (uint8_t*)cxplat_allocate(CXPLAT_POOL_FLAG_NON_PAGED, source->length, CXPLAT_TAG_UTF8_STRING);
...
{
    cxplat_free(string->value, CXPLAT_POOL_FLAG_NON_PAGED, CXPLAT_TAG_UTF8_STRING);
    string->value = NULL;
    string->length = 0;
}
