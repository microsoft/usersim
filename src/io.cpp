    ...
    _Inout_opt_ IRP* irp)
{
    ...
    mdl = reinterpret_cast<MDL*>(cxplat_allocate(CXPLAT_POOL_FLAG_NON_PAGED, sizeof(MDL), USERSIM_TAG_MDL));
    if (mdl == NULL) {
        return mdl;
    }
#pragma warning(push)
...
{
    auto io_work_item =
        (PIO_WORKITEM)cxplat_allocate(CXPLAT_POOL_FLAG_NON_PAGED, sizeof(IO_WORKITEM), USERSIM_TAG_IO_WORK_ITEM);
    if (!io_work_item) {
        return nullptr;
    }
    ...
    if (!CXPLAT_SUCCEEDED(status)) {
        cxplat_free(io_work_item, CXPLAT_POOL_FLAG_NON_PAGED, USERSIM_TAG_IO_WORK_ITEM);
        return nullptr;
    }
    return io_work_item;
}
...
IoFreeMdl(MDL* mdl)
{
    cxplat_free(mdl, CXPLAT_POOL_FLAG_NON_PAGED, USERSIM_TAG_MDL);
}
